...
static void create_collecter_beh(flecs::entity e)
{
    e.set(Blackboard{});
    BehNode* root =
        selector({
          sequence({
            _not(find_boost(e, "closest_pickup")),
            find_enemy(e, 1000.f, "attack_enemy"),
            move_to_entity(e, "attack_enemy")
          }),
            sequence({
          ...
          sequence({
            move_to_entity(e, "closest_pickup")
          })
            });
    e.set(BehaviourTree{ root });
}
...
static flecs::entity create_teammate(flecs::world& ecs, int x, int y, Color col, const char* texture_src)
{
    flecs::entity textureSrc = ecs.entity(texture_src);
    return ecs.entity()
        .set(Position{ x, y })
        ...
        .set(Team{ 0 })
        .set(NumActions{ 1, 0 })
        .set(MeleeDamage{ 100.f })
        .set(Blackboard{});
}

static flecs::entity create_monster_pickupper(flecs::world& ecs, int x, int y, Color col, const char* texture_src)
{
    flecs::entity textureSrc = ecs.entity(texture_src);
    return ecs.entity()
        .set(Position{ x, y })
...
static flecs::entity create_waypoint(flecs::world& ecs, int x, int y, flecs::entity way)
{
    return ecs.entity()
        .set(Position{ x, y })
        .set(WayPoint{ way });
...
void init_roguelike(flecs::world &ecs)
{
  ...
  create_collecter_beh(create_monster_pickupper(ecs, 5, 5, Color{0xee, 0x00, 0xee, 0xff}, "minotaur_tex"));
  ...
  flecs::entity way1 = create_waypoint(ecs, -10, -5);
  flecs::entity way2 = create_waypoint(ecs, -10, 5, way1);
  way1.set([&](WayPoint &way) {
      way.nextPoint = way2;
      });

  create_guard_beh(create_teammate(ecs, 1, 1, Color{ 88, 0, 255, 255 }, "swordsman_tex"));
}
...
static void process_actions(flecs::world &ecs)
{
  ...
  static auto playerPickup = ecs.query<const IsPickUpper, const Position, Hitpoints, MeleeDamage>();
  ...
  ecs.defer([&]
  {
    playerPickup.each([&](const IsPickUpper&, const Position &pos, Hitpoints &hp, MeleeDamage &dmg)
    {
