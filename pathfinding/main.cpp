...
static std::vector<Position> prev;
static float suboptimal_bound = std::numeric_limits<float>::max();

static std::vector<Position> find_path_ARA_star(const char* input, size_t width, size_t height, Position from, Position to, float weight)
{
    if (from.x < 0 || from.y < 0 || from.x >= int(width) || from.y >= int(height))
        return std::vector<Position>();
    ...
    float inflation_factor = weight;

    auto getG = [&](Position p) -> float { return g[coord_to_idx(p.x, p.y, width)]; };
    auto getF = [&](Position p) -> float { return getG(p) + inflation_factor * heuristic(p, to); };
    auto getF_without_weight = [&](Position p) -> float { return getG(p) + heuristic(p, to); };
    ...
    if(openList.empty())
        return std::vector<Position>();
    size_t bestIdx = find_bestScoreIdx();

    while (getF(to) > getF(openList[bestIdx]))
    {
        Position curPos = openList[bestIdx];
        openList.erase(openList.begin() + bestIdx);
        ...
        checkNeighbour({ curPos.x + 1, curPos.y });
        checkNeighbour({ curPos.x - 1, curPos.y });
        checkNeighbour({ curPos.x, curPos.y + 1 });
        checkNeighbour({ curPos.x, curPos.y - 1 });
    ...
    if (!openList.empty())
    {
        if (!inconsList.empty())
        {
            openList.insert(openList.end(), inconsList.begin(), inconsList.end());
            inconsList.clear();
        }

        bestIdx = 0;
        {
            float bestScore = getF_without_weight(openList[0]);
            for (size_t i = 1; i < openList.size(); ++i)
            {
                float score = getF_without_weight(openList[i]);
                if (score < bestScore)
                {
                    bestIdx = i;
                    bestScore = score;
        ...
        suboptimal_bound = std::min(inflation_factor, getG(to) / getG(openList[bestIdx]));

        printf("suboptimal_bound %.1f\n", suboptimal_bound);
...
static void reset_ara_star(size_t width, size_t height, Position from)
{
    ...
    eps = 10;
    suboptimal_bound = std::numeric_limits<float>::max();
    openList = { from };
    size_t inpSize = width * height;
    g.clear();
...
void draw_nav_data_ARA_star(const char* input, size_t width, size_t height, Position from, Position to, float weight)
{
    ...
    {
        eps -= 0.75f;
        if (eps <= 1.f || openList.empty() || suboptimal_bound <= 1.f)
            reset_ara_star(width, height, from);
        frames_until_next_ara_star_iteration = 60;
